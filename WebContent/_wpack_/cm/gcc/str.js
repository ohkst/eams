/*amd /cm/gcc/str.xml 16470 f3efc26576c0faba22f83101409c8b662a96a002624406df2eb88a3266d3f5a5 */
define({declaration:{A:{version:'1.0',encoding:'UTF-8'}},E:[{T:1,N:'html',A:{xmlns:'http://www.w3.org/1999/xhtml','xmlns:ev':'http://www.w3.org/2001/xml-events','xmlns:w2':'http://www.inswave.com/websquare','xmlns:xf':'http://www.w3.org/2002/xforms'},E:[{T:1,N:'head',A:{},E:[{T:1,N:'w2:type',E:[{T:3,text:'COMMON'}]},{T:1,N:'w2:buildDate'},{T:1,N:'w2:MSA'},{T:1,N:'xf:model',E:[{T:1,N:'w2:dataCollection',A:{baseNode:'map'}},{T:1,N:'w2:workflowCollection'}]},{T:1,N:'w2:layoutInfo'},{T:1,N:'w2:publicInfo',A:{method:'scwin.serialize,scwin.lpad,scwin.rpad,scwin.formatSSN,scwin.formatPhone,scwin.formatTime,scwin.getLocale,scwin.existKorean,scwin.isKorean,scwin.isFinalConsonant,scwin.attachPostposition,scwin.isBizID,scwin.isSSN,scwin.isEmail,scwin.isPhone,scwin.replaceAll,scwin.trim,scwin.getByteLength,scwin.__formatPhone'}},{T:1,N:'script',A:{lazy:'false',type:'text/javascript'},E:[{T:4,cdata:function(scopeObj){with(scopeObj){// =============================================================================
/**
 * 문자열 관련 함수를 작성한다.
 *
...
scwin.attachPostposition = function ($p, str) {
  if ($c.win.getLanguage($p) === "ko") {
    if ($c.str.isFinalConsonant($p, str)) {
      str = str + "은";
    } else {
      str = str + "는";
    }
...
scwin.isSSN = function ($p, str) {
  ...
  if (parseInt($c.date.getServerDateTime($p, "yyyy")) - parseInt(yy + str.substring(0, 2)) < 0) {
    return false;
  }

  // 외국인 주민번호 체크로직 추가
  if (str.charAt(6) != '5' && str.charAt(6) != '6' && str.charAt(6) != '7' && str.charAt(6) != '8') {
    if (temp == $c.num.parseInt($p, str.charAt(12))) {
      return true;
    } else {
      return false;
    }
  } else {
    if ((temp + 2) % 10 == $c.num.parseInt($p, str.charAt(12))) {
      return true;
    } else {
      return false;
    }
