/*amd /cm/gcc/win.xml 49331 b4f7da816407af3fafb1eaa083a7a5d105b433c9283e8a0497198643972f4ab5 */
...
scwin.getActiveWindowInfo = function ($p, scopeApi) {
  ...
  if (popupWindow !== null) {
    ...
    if (activeInfo["type"] == "T") {
      ...
      if (!$c.util.isEmpty($p, $c.data.getParameter($p, $p.main().pfm_layout.getWindow().$p, "menuInfo"))) {
        activeInfo["programCd"] = $c.data.getParameter($p, $p.main().pfm_layout.getWindow().$p, "menuInfo").programCd;
      }
...
scwin.showToastMessage = function ($p, messageType, message) {
  if ($c.util.isEmpty($p, $p.main().pfm_footer)) {
    return;
  }
  ...
  $c.util.setTimeout($p, function (idx) {
    ...
    $c.util.setTimeout($p, function (idx) {
      ...
      let objArr = $c.util.getChildren($p, pgfFooter.grp_noticeArea, {
  ...
  $c.util.setTimeout($p, function () {
    if ($p.isWFramePopup()) {
      ...
      if (func) {
        ...
        func = opener.$c.util.getCallBackFunction(opener.$p, callbackFnStr);
        if (func) {
          func($c.util.getJSON($p, callbackParamStr));
        }
      }
 ...
 * @example
 */
scwin.goHome = function ($p, param) {
  const contextPath = $c.sbm.getContextPath($p);
  if (contextPath == "" && !$c.util.isEmpty($p, param)) {
    top.window.location.href = "/?menuCd=" + param;
  } else if (contextPath == "" && $c.util.isEmpty($p, param)) {
 ...
 */
scwin.getFrame = function ($p, scopeApi) {
  try {
    if (typeof scopeApi == 'object') {
