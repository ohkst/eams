 ...
 */
scwin.addMinute = function ($p, pYmd, offset) {
 ...
 */
scwin.addHour = function ($p, pYmd, offset) {
 ...
 */
scwin.addDate = function ($p, pYmd, offset) {
 ...
 */
scwin.addMonth = function ($p, pYmd, offset) {
  try {
    let date = "";
    const isDate = $c.date.isDate($p, pYmd);
    if (!isDate) {
      return;
    }
    ...
    if (Number(dD) >= 30) {
      ...
      if (chkMonth1.indexOf(dM) > -1) {
        if ($c.date.isLeafYear($p, dY + "01" + "01")) {
          dD = "29";
        } else {
          dD = "28";
        }
 ...
 */
scwin.getServerDateTime = function ($p, sDateFormat) {
  if ($c.util.isEmpty($p, sDateFormat)) {
 ...
 */
scwin.formatDate = function ($p, str, type) {
  if (typeof $p == 'string') {
    type = str;
    str = $p;
...
scwin.__formatDate = function (str, type) {
  ...
  if (type == "slash") {
    ...
    for (let i = 0; i <= date.length - 1; i++) {
      ...
      if ((i + 1) % 2 == 0 && date.length - 1 !== i) output = output + "/";
    }
  } else if (type == "date") {
    if (date.length == 8) {
...
// return 예시) "12:34:02"
 */
scwin.formatTime = function ($p, value) {
 ...
 */
scwin.formatDateTime = function ($p, value, sDateFormat) {
  if ($c.util.isEmpty($p, sDateFormat)) {
    sDateFormat = "yyyyMMdd";
  }
 ...
 */
scwin.isDate = function ($p, sDate, timeChk) {
  let flag = true;
  try {
    if (sDate.length == 8) {
      sDate = sDate + "000000";
    ...
    if (isNaN(y1) || isNaN(m1) || isNaN(d1) || isNaN(h1) || isNaN(t1) || isNaN(s1)) flag = false;
    if (m1 < 1 || m1 > 12) flag = false;
    let maxDay = 31;
    if (m1 == 2) {
    ...
    if (h1 + t1 + s1 != "") {
      if (h1 < 0 || h1 > 24) {
        ...
        if (typeof timeChk == "boolean" && !timeChk) flag = false;
        if (t1 > 0) flag = false;
        if (s1 > 0) flag = false;
      }
      if (t1 < 0 || t1 > 59) flag = false;
      if (s1 < 0 || s1 > 59) flag = false;
    }
  } catch (ex) {
    console.error(ex);
    flag = false;
 ...
 */
scwin.diffDate = function ($p, fromDate, toDate) {
  try {
    if (!$c.date.isDate($p, fromDate) || !$c.date.isDate($p, toDate)) {
      return;
    }
 ...
 */
scwin.isLeafYear = function ($p, pYmd) {
  try {
    let isLeafYear = false;
    if (!$c.date.isDate($p, pYmd)) {
      return;
    } else {
 ...
 */
scwin.getDay = function ($p, value, type) {
  let dayVal;
  if (type == "num") {
 ...
 */
scwin.getLunar = function ($p, value) {
 ...
 */
scwin.getLastDateOfMonth = function ($p, yearMonth) {
  try {
    if (typeof yearMonth !== "string") {
      yearMonth = $c.str.serialize($p, yearMonth);
    }
