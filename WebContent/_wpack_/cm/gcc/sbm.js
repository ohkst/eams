/*amd /cm/gcc/sbm.xml 22945 91fd3540ec905e3c07d2645d33459282f2ebd901e9fbf540167a582cd410fe11 */
 ...
 */
scwin.getContextPath = function ($p) {
...
scwin.__preSubmitFunction = function (sbmObj) {
  if ($c.util.isEmpty($p, scwin.CONTEXT_PATH) === false && sbmObj.action.indexOf(scwin.CONTEXT_PATH) !== 0) {
...
scwin.__setActionParam = function (sbmObj) {
  ...
  if ($c.util.isEmpty($p, requestData)) {
    return;
  }
  if (typeof requestData === "string") {
    requestData = JSON.parse(requestData);
  }
  if ($c.util.isEmpty($p, sbmObj.__action)) {
  ...
  if ($c.util.isJSON($p, requestData)) {
    // DataMap에 저장된 데이터를 queryParam에 저장한다.
    for (let key in requestData) {
      if ($c.util.isEmpty($p, requestData[key]) === false && $c.util.isJSON($p, requestData[key]) === false) {
        queryParam[key] = encodeURIComponent(requestData[key]);
      } else if ($c.util.isJSON($p, requestData[key]) && $c.util.isArray($p, requestData[key]) === false) {
        for (let subKey in requestData[key]) {
          if ($c.util.isEmpty($p, requestData[key][subKey]) === false && $c.util.isJSON($p, requestData[key][subKey]) === false) {
...
scwin.__callbackSubmitFunction = function (resObj, sbmObj) {
  ...
  } else if (!$c.util.isEmpty($p, resObj.errorType) && typeof sbmObj._promise_submitErrorHandler === "function") {
    sbmObj._promise_submitErrorHandler(resObj);
  }
  const rsJSON = resObj.responseJSON || null;
  if (rsJSON && rsJSON.rsMsg) {
    $c.sbm.resultMsg($p, rsJSON.rsMsg);
  }
};
 ...
 */
scwin.execute = function ($p, sbmObj, requestData, compObj) {
  return new Promise(function (resolve, reject) {
    if (typeof sbmObj === "object") {
      ...
      if ($c.util.isEmpty($p, _sbmObj.action)) {
        const alertMsg = $c.data.getMessage($p, "MSG_CM_00002", "action");
        $c.win.alert($p, alertMsg);
        return false;
      }
      if ($c.util.isEmpty($p, _sbmObj.submitDoneHandler)) {
        _sbmObj._promise_submitDoneHandler = function (rtn) {
          resolve(rtn);
        };
      }
      if ($c.util.isEmpty($p, _sbmObj.submitErrorHandler)) {
 ...
 */
scwin.executeDynamic = function ($p, options, requestData, obj) {
  let submissionObj = $c.util.getComponent($p, options.id);
  if (submissionObj === null) {
    $c.sbm.create($p, options);
    submissionObj = $c.util.getComponent($p, options.id);
  } else {
    $p.deleteSubmission(options.id);
    $c.sbm.create($p, options);
    submissionObj = $c.util.getComponent($p, options.id);
  }
  return $c.sbm.execute($p, submissionObj, requestData, obj);
};
 ...
 */
scwin.executeWorkflow = function ($p, workflowObj) {
 ...
 */
scwin.create = function ($p, options) {
  let ref = options.ref || "";
  ...
  let submitHandler = typeof options.submitHandler === "function" ? options.submitHandler : typeof options.submitHandler === "string" ? options.submitHandler.indexOf("scwin") === 0 ? $p.id + options.submitHandler : options.submitHandler : "";
  let submitDoneHandler = typeof options.submitDoneHandler === "function" ? options.submitDoneHandler : typeof options.submitDoneHandler === "string" ? options.submitDoneHandler.indexOf("scwin") === 0 ? $p.id + options.submitDoneHandler : options.submitDoneHandler : "";
  let submitErrorHandler = typeof options.submitErrorHandler === "function" ? options.submitErrorHandler : typeof options.submitErrorHandler === "string" ? options.submitErrorHandler.indexOf("scwin") === 0 ? $p.id + options.submitErrorHandler : options.submitErrorHandler : "";
  let resJson = null;
  ...
  const submissionObj = {
    "id": options.id,
    // submission 객체의 ID. 통신 모듈 실행 시 필요.
    "ref": ref,
    // 서버로 보낼(request) DataCollection의 조건 표현식.(조건에 때라 표현식이 복잡하다) 또는 Instance Data의 XPath.
    "target": target,
    // 서버로 응답(response) 받은 데이터가 위치 할 DataCollection의 조건 표현식. 또는 Instance Data의 XPath.
    "action": action,
    // 통신 할 서버 측 URI.(브라우저 보안 정책으로 crossDomain은 지원되지 않는다.)
    "method": method,
    // [default: post, get, urlencoded-post] get:파라메타를 url에 붙이는 방식 (HTML과 동일).
    // post:파라메타를 body 구간에 담는 방식 (HTML과 동일). urlencoded-post:urlencoded-post.
    "mediatype": mediatype,
    // application/json
    "encoding": "UTF-8",
    // [default: utf-8, euc-kr, utf-16] 서버 측 encoding 타입 설정 (euc-kr/utf-16/utf-8)
    "mode": mode,
    // [default: synchronous, synchronous] 서버와의 통신 방식. asynchronous:비동기식. synchronous:동기식
    "processMsg": processMsg,
    // submission 통신 중 보여줄 메세지.
 ...
 */
scwin.getResultCode = function ($p, e) {
  let rsCode = $c.sbm.getMessageCode($p, 'STATUS_ERROR');
  try {
 ...
 */
scwin.getStatusMessage = function ($p, e) {
  try {
 ...
 * @exception 
 */
scwin.resultMsg = function ($p, resultData) {
  if ($c.util.isEmpty($p, resultData.statusCode)) {
    return;
  }
  ...
  switch (resultData.statusCode) {
    case msgCode.STATUS_ERROR:
      if (resultData.errorCode == "E0001") {
        const alertMsg = $c.data.getMessage($p, "MSG_CM_00003", message);
        $c.win.alert($p, alertMsg, function () {
          $c.win.reload($p);
        });
      } else if ($c.util.isEmpty($p, message) === false) {
        $c.win.alert($p, message);
      }
      break;
    case msgCode.STATUS_SUCCESS:
      if ($c.util.isEmpty($p, message) === false) {
        $c.win.showToastMessage($p, scwin.MESSAGE_CODE.STATUS_SUCCESS, message);
      }
      break;
    case msgCode.STATUS_INFO:
      $c.win.showToastMessage($p, scwin.MESSAGE_CODE.STATUS_INFO, message);
      break;
  }
};
 ...
 */
scwin.setAction = function ($p, sbmObj, sbmAction) {
  try {
    if (!$c.util.isEmpty($p, sbmObj)) {
 ...
 */
scwin.getServiceURL = function ($p) {
 ...
 * @example
 */
scwin.getMessageCode = function ($p, code) {
